...
class AlmanacTag(TransactionTestCase):
    ...
    def setUp(self):
        ...
        self.event1 = Event.objects.create(
            ...
            begin_time='14:20:00',
            end_time='16:52:00',
            content='AMD CPU is better than Intel`s. Even Apple discards Intel`s x86 Core and uses its own computer chips, M1. M1 chips are a lot faster than Intel`s CPUs.',
        ...
        self.event2 = Event.objects.create(
            ...
            begin_time='14:20:00',
            end_time='16:52:00',
            content='An apple is very delicious. It has more nutrients than orange, lemon, pineapples, grapes. Eat more apples and then you will be one step further from doctors.',
            last_editor_id=user2.id
        )
        ...
        self.event3 = Event.objects.create(
            ...
            begin_time='14:20:00',
            end_time='16:52:00',
            content='AMD and Intel fights to have more market share in CPU market. They both use x86 (AMD 64) architecture for their cores. The both companies earn a lot of money.',
            last_editor_id=user2.id
        )
        ...
        self.event4 = Event.objects.create(
            ...
            begin_time='14:20:00',
            end_time='16:52:00',
            content='Free market is very important element in capitalism. It makes the companies and workers more efficient and make more money.',
            last_editor_id=user2.id
        )
        ...
        self.event5 = Event.objects.create(
            ...
            begin_time='14:20:00',
            end_time='16:52:00',
            content='Orange is more delicious than lime, lemon, pineapple or apple and it is cheaper than the other vegetables or fruits. Eat more oranges.',
            last_editor_id=user2.id
        )
        ...
        self.event6 = Event.objects.create(
            ...
            begin_time='14:20:00',
            end_time='16:52:00',
            content='Intel`s CPU is more appropriate for gaming computers. It is the reason that still many computers use Intel`s Core CPU. x86 CPUs also have more compatibility.',
            last_editor_id=user2.id
        )
        ...
        self.event7 = Event.objects.create(
            ...
            begin_time='14:20:00',
            end_time='16:52:00',
            content='Since there was a hurricane this year, under the law of capitalism, the value of apples rose higher than oranges or limes. We need more money to buy oranges.',
            last_editor_id=user2.id
        )
...
class AlmanacGroup(TransactionTestCase):
    ...
    def test_filtered_group1(self):
        ...
        response = client.put('/api/group/filtered/', json.dumps({
            'filter_options': {},
            'sort_options': [],
            'count_options': {}
        }),
        ...
        response = client.post('/api/group/filtered/', json.dumps({
            'filter_options': {},
            'sort_options': [],
            'count_options': {}
        }),
        ...
        self.assertEqual(len(response.json()), 5)
        self.assertEqual(response.json()[0]['id'], self.group1.id)
        self.assertEqual(response.json()[1]['id'], self.group2.id)
        self.assertEqual(response.json()[2]['id'], self.group3.id)
        self.assertEqual(response.json()[3]['id'], self.group4.id)
        self.assertEqual(response.json()[4]['id'], self.group5.id)

    def test_filtered_group2(self):
        ...
        response = client.post('/api/group/filtered/', json.dumps({
            'filter_options': {},
            'sort_options': ['notifications'],
            'count_options': {}
        }),
        content_type='application/json')
        self.assertEqual(len(response.json()), 5)
        self.assertEqual(response.json()[0]['id'], self.group1.id)
        self.assertEqual(response.json()[1]['id'], self.group3.id)
        self.assertEqual(response.json()[2]['id'], self.group2.id)
        self.assertEqual(response.json()[3]['id'], self.group4.id)
        self.assertEqual(response.json()[4]['id'], self.group5.id)

        response = client.post('/api/group/filtered/', json.dumps({
            'filter_options': {},
            'sort_options': ['members'],
            'count_options': {}
        }),
        content_type='application/json')
        self.assertEqual(len(response.json()), 5)
        self.assertEqual(response.json()[0]['id'], self.group1.id)
        self.assertEqual(response.json()[1]['id'], self.group5.id)
        self.assertEqual(response.json()[2]['id'], self.group2.id)
        self.assertEqual(response.json()[3]['id'], self.group3.id)
        self.assertEqual(response.json()[4]['id'], self.group4.id)
    ...
    def test_filtered_group3(self):
        ...
        self.assertEqual(response.json()[1]['id'], self.group3.id)
        self.assertEqual(response.json()[2]['id'], self.group4.id)
        ...
        self.assertEqual(response.json()[0]['id'], self.group1.id)
        self.assertEqual(response.json()[1]['id'], self.group2.id)
        self.assertEqual(response.json()[2]['id'], self.group5.id)
        ...
        content_type='application/json')
        self.assertEqual(len(response.json()), 1)
        self.assertEqual(response.json()[0]['id'], self.group3.id)
    ...
    def test_filtered_group4(self):
        ...
        content_type='application/json')
        self.assertEqual(len(response.json()), 2)
        self.assertEqual(response.json()[0]['id'], self.group4.id)
        self.assertEqual(response.json()[1]['id'], self.group5.id)
...
class AlmanacEventReport(TransactionTestCase):
    ...
    def test_get_event_report_by_event(self):
        ...
        response = client.head('/api/event_report/event/{}/'.format(id_event))
        self.assertEqual(response.status_code, 405)

        response = client.get('/api/event_report/event/{}/'.format(id_wrong))
        self.assertEqual(response.status_code, 404)

        response = client.get('/api/event_report/event/{}/'.format(id_event))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.json()), 1)
        self.assertEqual(response.json()[0]['event'], self.event1.id)
        self.assertEqual(response.json()[0]['reporter'], self.user2.id)
    ...
    def test_get_group_report_by_group(self):
        ...
        response = client.head('/api/group_report/group/{}/'.format(id_group))
        self.assertEqual(response.status_code, 405)

        response = client.get('/api/group_report/group/{}/'.format(id_wrong))
        self.assertEqual(response.status_code, 404)

        response = client.get('/api/group_report/group/{}/'.format(id_group))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.json()), 1)
        self.assertEqual(response.json()[0]['group'], self.group1.id)
        self.assertEqual(response.json()[0]['reporter'], self.user2.id)
